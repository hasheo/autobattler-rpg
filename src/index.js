...
const SHOP = [
  {
    ...
    effectsByTier: {
      0: [],
      1: [{ apply: (_, owner) => { owner.critChance += 0.1; } }],
      2: [{ apply: (_, owner) => { owner.lifesteal += 0.15; } }],
      3: [{ apply: (_, owner) => { owner.critChance += 0.35; } }]
    }
  },
  {
    id: "guardian-shield",
    ...
    effectsByTier: {
      0: [],
      1: [{ apply: (_, owner) => { owner.shield += 18; } }],
      2: [{ apply: (_, owner) => { owner.defense += 20; } }],
      3: [{ apply: (_, owner, enemy) => { enemy.stunTimer = Math.max(enemy.stunTimer, 2); } }]
    }
  },
  {
    id: "ember-staff",
    ...
    baseDefense: 1,
    effectsByTier: {
      0: [{ apply: (_, owner) => { owner.burnPerSecond += 1; } }],
      1: [{ apply: (_, owner) => { owner.burnPerSecond += 2; } }],
      2: [{ apply: (_, owner) => { owner.burnPerSecond += 4; } }],
      3: [{ apply: (_, owner) => { owner.burnPerSecond += 8; } }]
    }
  },
  {
    id: "vampire-dagger",
    ...
    baseDefense: 0,
    effectsByTier: {
      0: [{ apply: (_, owner) => { owner.lifesteal += 0.08; } }],
      1: [{ apply: (_, owner) => { owner.lifesteal += 0.16; } }],
      2: [{ apply: (_, owner) => { owner.lifesteal += 0.24; } }],
      3: [{ apply: (_, owner) => { owner.lifesteal += 0.4; } }]
    }
  }
];
...
function upgradeIfPossible(player, blueprint) {
  ...
  for (let tier = 0; tier < 3; tier += 1) {
    while (tiers[tier] >= 3) {
      ...
      console.log(`  ↳ ${player.name} upgraded ${blueprint.name} to tier ${tier + 1}!`);
    }
  }
...
function buyItem(player) {
  ...
  console.log(`  ${player.name} bought ${item.name} (tier 0) for ${item.baseCost}g.`);
  upgradeIfPossible(player, item);
}

function preparationPhase(player, day) {
  const income = incomeForLevel(player.level);
  player.gold += income;
  console.log(`${player.name} preparation (Day ${day}): +${income}g income (gold=${player.gold}).`);

  while (player.gold >= levelUpCost(player.level) && player.level < 10 && Math.random() < 0.6) {
    const cost = levelUpCost(player.level);
    player.gold -= cost;
    player.level += 1;
    console.log(`  ${player.name} leveled up to ${player.level} (cost ${cost}g).`);
  }
...
function createFighter(player) {
  ...
  for (const item of fighter.items) {
    ...
    for (const effect of tierEffects) {
      effect.apply({ timeSec: 0, random: Math.random }, fighter, fighter);
    }
  }
...
function dealDamage(source, target, rawDamage, random) {
  let damage = Math.max(1, rawDamage - target.defense * 0.35);
  if (random() < source.critChance) damage *= source.critMultiplier;
...
function applyBurn(attacker, defender) {
  if (attacker.burnPerSecond > 0) defender.hp -= attacker.burnPerSecond;
}
...
function battle(playerA, playerB) {
  ...
  while (timeSec < BATTLE_TIME_LIMIT && a.hp > 0 && b.hp > 0) {
    if (a.stunTimer <= 0) {
      dealDamage(a, b, a.attack, random);
      applyBurn(a, b);
    } else {
      a.stunTimer -= 1;
    }
    ...
    if (b.stunTimer <= 0) {
      dealDamage(b, a, b.attack, random);
      applyBurn(b, a);
    } else {
      b.stunTimer -= 1;
    }
...
function printInventory(player) {
  ...
  console.log(`${player.name} loadout: ${summary || "No items"}`);
}
...
function runGame() {
  ...
  for (let day = 1; day <= DAYS; day += 1) {
    console.log(`\n=== Day ${day} ===`);
    preparationPhase(p1, day);
    preparationPhase(p2, day);
    ...
    console.log(`Score => ${p1.name}: ${p1Wins} | ${p2.name}: ${p2Wins}`);
  }

  console.log("\n=== Final Result ===");
  if (p1Wins === p2Wins) console.log(`It's a draw after ${DAYS} days (${p1Wins}-${p2Wins}).`);
  else {
    const champion = p1Wins > p2Wins ? p1.name : p2.name;
    console.log(`${champion} wins the campaign ${Math.max(p1Wins, p2Wins)}-${Math.min(p1Wins, p2Wins)}.`);
  }
}
